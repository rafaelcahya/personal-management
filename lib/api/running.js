...
export async function getVo2maxStat() {
  const res = await fetch('/api/running/v1/analytics/vo2max-stat')
  if (res.status === 401) throw new Error('UNAUTHORIZED')
  const json = await res.json()
  if (!res.ok) throw new Error(json.error || 'Failed to fetch VO2max stat')
  return json.data
}
