...
export async function fetchWeeklyStatsForWeek(activityType = null, weeksAgo = 0) {
  const params = new URLSearchParams({
    tz_offset: String(new Date().getTimezoneOffset()),
    weeks_ago: String(weeksAgo),
  })
  if (activityType) params.set('type', activityType)
  const res = await fetch(`/api/running/v1/dashboard/weekly-stats?${params}`)
  if (res.status === 401) throw new Error('UNAUTHORIZED')
  const data = await res.json()
  if (!res.ok) throw new Error(data.error || 'Failed to fetch weekly stats')
  return data.data
}
...
export async function fetchSavedRoutes() {
  const res = await fetch('/api/running/v1/routes')
  if (res.status === 401) throw new Error('UNAUTHORIZED')
  const json = await res.json()
...
export async function deleteSavedRoute(id) {
  const res = await fetch(`/api/running/v1/routes/${id}`, { method: 'DELETE' })
  if (res.status === 401) throw new Error('UNAUTHORIZED')
  if (!res.ok) {
    const json = await res.json()
...
export async function updateSavedRoute(id, { name }) {
  const res = await fetch(`/api/running/v1/routes/${id}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export async function exportRouteGpx(id) {
  const res = await fetch(`/api/running/v1/routes/${id}/export/gpx`)
  if (res.status === 401) throw new Error('UNAUTHORIZED')
  if (!res.ok) {
...
export async function getVo2maxTarget() {
  const res = await fetch('/api/running/v1/analytics/vo2max-target')
  if (res.status === 401) throw new Error('UNAUTHORIZED')
  const json = await res.json()
  if (!res.ok) throw new Error(json.error || 'Failed to fetch VO2max target')
  return json.data
}

export async function updateVo2maxTarget(vo2maxTarget) {
  const res = await fetch('/api/running/v1/analytics/vo2max-target', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ vo2max_target: vo2maxTarget }),
  })
  if (res.status === 401) throw new Error('UNAUTHORIZED')
  const json = await res.json()
  if (!res.ok) throw new Error(json.error || 'Failed to update VO2max target')
  return json.data
}
