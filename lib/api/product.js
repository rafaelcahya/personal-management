...
export async function updateStockEntry(entryId, payload) {
  const res = await fetch(`/api/inventory/v1/product/stock/${entryId}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  })
  ...
  if (!res.ok) {
    const errorMsg = Array.isArray(data.error) ? data.error.join(', ') : data.error
    throw new Error(errorMsg || 'Failed to update stock entry')
  }
  return data.data
}
...
export async function deleteStockEntry(entryId) {
  ...
  if (!res.ok) {
    const errorMsg = Array.isArray(data.error) ? data.error.join(', ') : data.error
    throw new Error(errorMsg || 'Failed to delete stock entry')
  }
  return data.data
}
