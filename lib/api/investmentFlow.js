...
export async function getInvestmentFlowTree() {
  ...
  return data.data || { nodes: [], uninvestedCash: 0, cashCategories: [], highlights: {} }
}
...
export async function closePosition(id, payload) {
  ...
  if (!res.ok) {
    const err = new Error(data.error || 'Failed to close position')
    if (data.code) err.code = data.code
    throw err
  }
...
export async function updateHighlights(highlights) {
  ...
  if (!res.ok) {
    throw new Error(data.error || 'Failed to update highlights')
  }
