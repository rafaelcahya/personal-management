...
export async function fetchImportSuggestions() {
  const res = await fetch('/api/trade/v1/event/import-suggestions', { cache: 'no-store' })
  const data = await res.json()
  if (!res.ok) throw new Error(data.error || 'Failed to fetch import suggestions')
  return data.data ?? []
}
