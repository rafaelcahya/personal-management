...
export async function getZoneAnalytics(
  ...
  const { fromDate, toDate } = getDateRange(range, startDate, endDate, tzOffsetMs)
...
const ZONE_GUIDANCE = [
  'Easy aerobic base. Fully conversational — use for warm-up, cool-down, and active recovery.',
  'Foundation training. Comfortable and sustainable — the bulk of your weekly mileage.',
  'Comfortably hard. Improves lactate clearance and running economy over time.',
  'Hard, controlled effort. Pushes your lactate threshold — threshold runs and cruise intervals.',
  'Maximum effort. Brief, intense intervals to boost aerobic capacity ceiling.',
]

export async function getZoneReference(supabase, userId) {
  const [profileResult, settingsResult] = await Promise.all([
    supabase
      .from('rt_users')
      .select('max_hr, resting_hr_baseline, threshold_hr, threshold_pace_sec')
  ...
  if (profileResult.error)
    console.warn('[getZoneReference] rt_users query failed:', profileResult.error.message)
  if (settingsResult.error)
    console.warn('[getZoneReference] rt_user_settings query failed:', settingsResult.error.message)
  ...
  const zones = HR_ZONE_LABELS.map((label, i) => ({
    zone: `Z${i + 1}`,
    name: label.replace(/^Z\d\s/, ''),
    hr: hrBoundaries?.[i] ? { min: hrBoundaries[i].min, max: hrBoundaries[i].max } : null,
    pace: paceBoundaries?.[i] ? { min: paceBoundaries[i].min, max: paceBoundaries[i].max } : null,
    guidance: ZONE_GUIDANCE[i],
  }))

  return {
    zones,
    hr_method: hrMethod,
    hr_configured: hrBoundaries !== null,
    pace_configured: paceBoundaries !== null,
  }
}
