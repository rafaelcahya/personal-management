import { getFromDate } from './analyticsRangeUtils'
...
export async function getZoneAnalytics(supabase, userId, range, activityType) {
  const fromDate = getFromDate(range)

  // 1. User profile for HR zone config
  const [profileResult, settingsResult] = await Promise.all([
    supabase.from('rt_users').select('max_hr, resting_hr_baseline').eq('id', userId).maybeSingle(),
    supabase
      .from('rt_user_settings')
      .select('hr_zones_method, threshold_hr, threshold_pace_sec')
      .eq('user_id', userId)
      .maybeSingle(),
  ])
  ...
  const thresholdHr = settingsResult.data?.threshold_hr ?? null
  const thresholdPaceSec = settingsResult.data?.threshold_pace_sec ?? null
