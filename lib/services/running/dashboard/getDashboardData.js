...
import { fetchStreak } from '@/lib/services/running/analytics/getStreak'
...
async function fetchWeeklyStats(
  ...
  const upperBound = currentEnd ?? new Date()
...
async function fetchTrainingLoad(supabase, userId, now) {
  ...
  const tsb = status === 'no_data' ? null : Math.round((chronic_load_28d - acute_load_7d) * 10) / 10
...
export async function getWeeklyStatsOnly(
  userId,
  { activityType = null, tzOffsetMs = 0, weeksAgo = 0 } = {}
) {
  const supabase = await createClient()
  const now = new Date()
  const localNow = new Date(now.getTime() - tzOffsetMs)

  // Selected week window (7 days only — not cumulative)
  const localCurrentWeekStart = startOfWeek(subWeeks(localNow, weeksAgo), { weekStartsOn: 1 })
  const localPrevWeekStart = startOfWeek(subWeeks(localNow, weeksAgo + 1), { weekStartsOn: 1 })
  ...
  // For past weeks, cap upper bound at the end of that week (not "now")
  const currentEnd =
    weeksAgo === 0 ? now : new Date(currentWeekStart.getTime() + 7 * 24 * 60 * 60 * 1000)
 ...
 *   streak: { current_weeks: number, best_weeks: number, at_risk: boolean, active_this_week: boolean, unit: 'week' }
 * }>}
 */
export async function getDashboardData(
  ...
  // Cached callers pass an admin client (unstable_cache can't read cookies); every
  // query below filters by user_id explicitly, so bypassing RLS is safe here.
