...
async function fetchNextRaceGoal(supabase, userId, now) {
  const today = format(now, 'yyyy-MM-dd')

  const { data, error } = await supabase
    .from('rt_goals')
    .select('target_distance_m, target_date')
    .eq('user_id', userId)
    .eq('goal_type', 'race')
    .eq('status', 'active')
  ...
  const days_until = Math.ceil(
    (new Date(data.target_date).getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
  )
 ...
 */
async function fetchCalendarActivities(supabase, userId, now) {
 ...
 */
async function fetchHealthToday(supabase, userId, now) {
  const today = format(now, 'yyyy-MM-dd')
 ...
 * }>}
 */
export async function getDashboardData(userId, activityType = null) {
  ...
  const [
    ...
    fetchNextRaceGoal(supabase, userId, now),
    fetchRecentActivities(supabase, userId, activityType),
    fetchCalendarActivities(supabase, userId, now),
    fetchHealthToday(supabase, userId, now),
  ])

  return {
    weekly_stats,
    training_load: { ...training_load, ...weekly_load, effort_split, next_race_goal },
    recent_activities,
    calendar_activities,
    health_today,
  }
