...
export async function buildInsightContext(activityId, userId) {
  ...
  const [splits, recentActivities, trainingLoad, healthLog, goals, profile] = await Promise.all([
...
function serializeToPlainText({ activity, splits, recentActivities, trainingLoad, healthLog, goals, profile }) {
  const lines = []
  ...
  if (recentActivities.length > 0) {
    ...
    for (const a of recentActivities) {
      const date = a.started_at ? new Date(a.started_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' }) : ''
    ...
    const avgPaceSec = recentActivities
      .map((a) => a.avg_pace_sec_per_km)
      .filter(Boolean)
    const avgHr = recentActivities.map((a) => a.avg_hr).filter(Boolean)
    const avgPaceFormatted = avgPaceSec.length > 0
      ? formatPace(Math.round(avgPaceSec.reduce((a, b) => a + b, 0) / avgPaceSec.length))
      : null
    const avgHrFormatted = avgHr.length > 0
      ? Math.round(avgHr.reduce((a, b) => a + b, 0) / avgHr.length)
      : null
    const baselineParts = [
  ...
  if (trainingLoad) {
    ...
    const loadParts = [
      ...
      trainingLoad.chronic_load_28d != null ? `Chronic load 28d: ${trainingLoad.chronic_load_28d}` : null,
    ].filter(Boolean).join(' | ')
