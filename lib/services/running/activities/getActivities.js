...
export async function getActivities(
  userId,
  { from, to, type, sort = 'newest', page = 1, limit = 20 } = {}
) {
  ...
  let query = supabase
    .from('rt_activities')
    .select(
      'id, started_at, distance_m, duration_sec, moving_time_sec, avg_pace_sec_per_km, avg_hr, activity_type, source, name, elevation_gain_m, pr_count, workout_type, avg_cadence, avg_temp_c, calories, avg_watts, weighted_avg_watts, device_watts, summary_polyline, gear_id',
      { count: 'exact' }
    )
    .eq('user_id', userId)
