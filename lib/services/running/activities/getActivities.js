...
export async function getActivities(
  ...
  let query = supabase
    .from('rt_activities')
    .select(
      'id, started_at, distance_m, duration_sec, moving_time_sec, avg_pace_sec_per_km, avg_hr, activity_type, source, name, elevation_gain_m, pr_count, workout_type, avg_cadence, avg_temp_c, calories, gear_id',
      { count: 'exact' }
    )
    .eq('user_id', userId)
  ...
  const [gearResult, beResult] = await Promise.all([
    gearIds.length > 0
      ? supabase.from('rt_gear').select('id, name, distance_m').in('id', gearIds)
  ...
  const gearMap = new Map((gearResult.data || []).map((g) => [g.id, g]))

  const beRows = beResult.data ?? []
  ...
  if (beRows.length > 0) {
    ...
    // Dependent follow-up — needs the names/ranks resolved above, so stays sequential
    ...
    for (const row of beRows) {
      ...
      if (globalLatestMap.get(key)?.activity_id === row.activity_id) {
        ...
        bestEffortsMap.get(row.activity_id).set(row.name, { name: row.name, pr_rank: row.pr_rank })
      }
    }
  }
