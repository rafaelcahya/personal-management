import { createAdminClient } from '@/lib/supabase/admin'

const GATE_DURATION_SEC = 20 * 60

/**
 * Computes and persists derived metrics (EF, aerobic decoupling, VO2max)
 * for a single activity. Safe to call multiple times — always overwrites.
 * Caller is responsible for verifying that activityId belongs to userId
 * before calling — this function does not perform an auth check internally.
 *
 * @param {string} activityId - rt_activities.id (UUID)
 * @param {string} userId - auth user id, used for ownership scoping on DB writes
 * @returns {{ updated: boolean, fields: string[], values: object }}
 */
export async function computeAndSaveDerivedMetrics(activityId, userId) {
  try {
    const supabase = createAdminClient()

    const [{ data: act }, { data: userProfile }, { data: hrFallback }] = await Promise.all([
      supabase
        .from('rt_activities')
        .select('avg_hr, moving_time_sec, duration_sec, raw_data')
        .eq('id', activityId)
        .eq('user_id', userId)
        .single(),
      supabase.from('rt_users').select('max_hr').eq('id', userId).maybeSingle(),
      supabase
        .from('rt_activities')
        .select('max_hr')
        .eq('user_id', userId)
        .not('max_hr', 'is', null)
        .order('max_hr', { ascending: false })
        .limit(1)
        .maybeSingle(),
    ])

    if (!act) return { updated: false, fields: [], values: {} }

    const durationSec = act.moving_time_sec ?? act.duration_sec ?? 0
    const avgHr = act.avg_hr

    if (durationSec <= GATE_DURATION_SEC || !avgHr) {
      return { updated: false, fields: [], values: {} }
    }

    const hrMax = userProfile?.max_hr ?? hrFallback?.max_hr ?? null

    const updates = {}

    // ── Efficiency Factor ──────────────────────────────────────────────────────
    const avgSpeedMs = act.raw_data?.average_speed ?? null
    if (avgSpeedMs != null && avgHr > 0) {
      updates.efficiency_factor = parseFloat((avgSpeedMs / avgHr).toFixed(4))
    }

    // ── Aerobic Decoupling (Pa:Hr) ─────────────────────────────────────────────
    const { data: streams } = await supabase
      .from('rt_activity_streams')
      .select('pace_sec_per_km, heart_rate')
      .eq('activity_id', activityId)
      .not('pace_sec_per_km', 'is', null)
      .not('heart_rate', 'is', null)
      .order('timestamp', { ascending: true })

    if (Array.isArray(streams) && streams.length >= 4) {
      const midpoint = Math.floor(streams.length / 2)
      const firstHalf = streams.slice(0, midpoint)
      const secondHalf = streams.slice(midpoint)

      // Wrap in Number() to handle Postgres numeric-as-string from PostgREST
      const avg = (arr, key) => arr.reduce((s, r) => s + Number(r[key]), 0) / arr.length

      const firstPace = avg(firstHalf, 'pace_sec_per_km')
      const firstHr = avg(firstHalf, 'heart_rate')
      const secondPace = avg(secondHalf, 'pace_sec_per_km')
      const secondHr = avg(secondHalf, 'heart_rate')

      if (firstPace > 0 && firstHr > 0 && secondPace > 0 && secondHr > 0) {
        const firstRatio = 1 / firstPace / firstHr
        const secondRatio = 1 / secondPace / secondHr
        const decoupling = (firstRatio / secondRatio - 1) * 100

        if (Number.isFinite(decoupling)) {
          updates.aerobic_decoupling = parseFloat(decoupling.toFixed(2))
        }
      }
    }

    // ── Estimated VO2max (HR-based submax formula) ────────────────────────────
    // VO2max = (speed_m_per_min × 0.2 + 3.5) / (avgHR / HRmax)
    // Uses HRmax from user settings (rt_users.max_hr), fallback to highest
    // recorded max_hr across all activities. Requires HRmax to calculate.
    if (avgSpeedMs != null && avgHr > 0 && hrMax != null) {
      const avgSpeedMPerMin = avgSpeedMs * 60
      const vo2AtPace = avgSpeedMPerMin * 0.2 + 3.5
      const percentVO2max = avgHr / hrMax
      const raw = vo2AtPace / percentVO2max
      if (raw >= 20 && raw <= 90) {
        updates.estimated_vo2max = parseFloat(raw.toFixed(2))
      }
    }

    if (Object.keys(updates).length === 0) return { updated: false, fields: [], values: {} }

    const { error } = await supabase
      .from('rt_activities')
      .update(updates)
      .eq('id', activityId)
      .eq('user_id', userId)

    if (error) {
      console.error('[computeAndSaveDerivedMetrics] update failed', { activityId })
      throw new Error('Derived metrics update failed')
    }

    return { updated: true, fields: Object.keys(updates), values: updates }
  } catch (err) {
    console.error('[computeAndSaveDerivedMetrics] failed', { activityId, error: err })
    return { updated: false, fields: [], values: {}, error: err.message ?? String(err) }
  }
}
