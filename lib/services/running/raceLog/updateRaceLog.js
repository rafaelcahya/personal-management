...
export async function updateRaceLog(supabase, userId, raceLogId, payload) {
  ...
  const { data, error } = await supabase
    ...
    .eq('user_id', userId)
    .select(
      'id, title, race_date, distance_m, finish_time_sec, avg_pace_sec_per_km, avg_hr, elevation_gain_m, position_place, position_male, did_not_finish, activity_id, notes, created_at, updated_at'
    )
    .single()
