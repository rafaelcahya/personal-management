...
export async function getProductHistoryList(
  ...
  let query = supabase
    ...
    .select(
      'id, uuid, product, brand, type, status, quantity, remaining_quantity, depleted_quantity, note, product_image, start_usage_date, end_usage_date, product_list_id, user_id, created_at, updated_at, deleted_at',
      { count: 'exact' }
    )
    .eq('user_id', userId)
