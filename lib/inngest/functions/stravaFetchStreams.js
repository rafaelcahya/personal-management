...
export const stravaFetchStreams = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run('compute-derived-metrics', async () => {
      ...
      // Fetch the activity row to get avg_hr, moving_time_sec, avg_pace_sec_per_km, avg_speed
      const { data: act } = await supabase
        .from('rt_activities')
        .select('avg_hr, moving_time_sec, duration_sec, avg_pace_sec_per_km, raw_data')
        .eq('id', activityId)
        .single()
      ...
      // Gate: duration > 20 min AND HR data available
      const GATE_DURATION_SEC = 20 * 60
      if (durationSec <= GATE_DURATION_SEC || !avgHr) return

      const updates = {}

      // ── Efficiency Factor ────────────────────────────────────────────────────
      // EF = avg_speed_ms / avg_hr  (higher = better running economy)
      ...
      // Requires pace stream + HR stream — split at 50% of duration
      // Fetch stream rows for this activity ordered by timestamp
      ...
      if (Array.isArray(streams) && streams.length >= 4) {
        ...
        // Pa:Hr uses speed (inverse of pace) for the ratio
        // pace_sec_per_km is inverse of speed → use it inverted: speed proxy = 1/pace
      ...
      const avgSpeedMsForVo2 = act.raw_data?.average_speed ?? null
      if (avgSpeedMsForVo2 != null && avgHr > 0) {
        ...
        if (userRow?.max_hr) {
          const avgSpeedMPerMin = avgSpeedMsForVo2 * 60
