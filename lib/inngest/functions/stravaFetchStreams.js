...
import { buildPrAchievement } from '@/lib/services/running/notifications/detectPrAchievement'
...
export const stravaFetchStreams = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (credentials.needs_reconnect) {
      console.warn(`[stravaFetchStreams] user ${userId} needs reconnect — skipping`)
      return
    }
    ...
    if (tokenResult.skipped || tokenResult.needsReconnect) {
      console.warn(`[stravaFetchStreams] token refresh skipped for user ${userId}`)
      return
    }
    ...
    // Step 5b: Fire a PR notification when this sync produced a new personal record.
    // Best efforts are re-inserted on every re-sync, so pr_notified_at guards against
    // duplicate notifications for the same activity.
    await step.run('notify-pr-achievement', async () => {
      const payload = buildPrAchievement(stravaActivity.best_efforts, activityId)
      if (!payload) return { skipped: true, reason: 'no_pr' }

      const supabase = createAdminClient()

      const { data: activity } = await supabase
        .from('rt_activities')
        .select('pr_notified_at')
        .eq('id', activityId)
        .eq('user_id', userId)
        .maybeSingle()

      if (activity?.pr_notified_at) return { skipped: true, reason: 'already_notified' }
      ...
      if (settings?.push_notifications_enabled && settings?.push_subscription) {
        ...
        if (result.expired) await savePushSubscription(supabase, userId, null)
      }

      await supabase
        .from('rt_activities')
        .update({ pr_notified_at: new Date().toISOString() })
        .eq('id', activityId)
        .eq('user_id', userId)

      return { notified: true, count: payload.data.distances.length }
    })
    ...
    await step.run('fetch-and-store-streams', async () => {
      ...
      const rows = timeData.map((t, i) => {
        ...
        return {
          ...
          pace_sec_per_km: v >= 0.1 ? Math.round(1000 / v) : null,
        }
      })
    ...
    // Step 11: Mark enrichment complete — only set after all prior steps succeed
    ...
    // Step 12: Refresh cached dashboard/analytics so the new activity shows up.
    // Guarded inside the helper — if revalidateTag isn't available here, the cache
    // TTL is the backstop.
    await step.run('invalidate-analytics-cache', async () => {
      return invalidateRunningAnalytics(userId)
    })
  }
)
