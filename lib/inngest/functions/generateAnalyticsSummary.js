...
export const generateAnalyticsSummary = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const isDuplicate = await step.run('check-duplicate', async () => {
      ...
      if (triggeredByActivityId) {
        // WB-4: activity-triggered — check by activity ID
        const { data } = await supabase
          .from('rt_ai_insights')
          .select('id')
          .eq('user_id', userId)
          .eq('insight_type', 'analytics_summary')
          .contains('data_refs', { triggered_by_activity_id: triggeredByActivityId })
          .limit(1)
          .maybeSingle()
        return !!data
      }

      // WB-4: manual trigger — check for a pending row created in the last 5 minutes
    ...
    await step.run('insert-pending', async () => {
      ...
      if (error) throw new Error('Insert pending rows failed')
    })
    ...
    await step.run('save-insights', async () => {
      ...
      // S-5: upsert completed rows over the pending sentinel rows
      const { error } = await supabase
        .from('rt_ai_insights')
        .upsert(rows, { onConflict: 'user_id,insight_type,data_refs' })
      // S-5: do not expose raw DB error message to logs visible to callers
