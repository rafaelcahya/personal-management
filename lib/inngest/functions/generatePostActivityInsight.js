...
import { buildPostActivityPrompt } from '@/lib/services/running/ai/prompts'
  ...
  async ({ event, step }) => {
    ...
    const isDuplicate = await step.run('check-duplicate', async () => {
      ...
      const { data } = await supabase
        ...
        .contains('data_refs', { activity_id: activityId })
        .maybeSingle()
      return !!data
    })
    ...
    const { output, isValid } = await step.run('call-claude', async () => {
      const systemPrompt = buildPostActivityPrompt(profile)
    ...
    await step.run('save-insight', async () => {
      ...
      const { error } = await supabase.from('rt_ai_insights').insert({
        ...
        content: output || null,
        data_refs: { activity_id: activityId },
        retry_count: 0,
      })
