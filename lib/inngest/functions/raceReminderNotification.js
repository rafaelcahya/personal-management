...
export const raceReminderNotification = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    for (const race of races) {
      ...
      await step.run(`save-notification-${shortId}-${key}d`, async () => {
        const supabase = createAdminClient()
        await createNotification(supabase, {
          ...
          message: msg.body,
          data: { race_id: raceId, days_until: daysUntil, url: '/main/running/race-log' },
        })
      })

      let pushResult = { sent: false }
      if (pushEnabled) {
        pushResult = await step.run(`send-push-${shortId}-${key}d`, async () => {
