...
export const checkAnomaly = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    for (const anomaly of anomalies) {
      const anomalyResult = await step.run(`handle-anomaly-${anomaly.type}`, async () => {
        ...
        if (anomaly.severity !== 'info') {
          await createNotification(supabase, {
            ...
            type: 'anomaly',
            title: ANOMALY_TITLES[anomaly.type] ?? 'Anomaly Detected',
            message: 'A training anomaly was detected. Tap to review your recent activity.',
            data: {
          ...
          if (
            ...
            return { notified: pushResult.sent, ...pushResult }
          }
        }
      })

      if (anomalyResult?.expired) subscriptionExpired = true
    }
