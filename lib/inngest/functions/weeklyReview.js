...
export const weeklyReview = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const userId = await step.run('get-user', async () => {
      ...
      let query = supabase.from('rt_users').select('id')
    ...
    await step.run('save-notification', async () => {
      ...
      await createNotification(supabase, {
        ...
        type: 'weekly_review',
        title: 'Weekly Training Review',
        message: 'Your weekly running summary is ready. Tap to read.',
        data: { week_start: weekRange.weekStart, url: '/main/running/ai' },
      })
    })
