...
- 2026-09-05: **Notifications API test pattern (#801)** — `before()` vs `beforeEach()` matters for `cy.setupApiAuthCookies()`: Cypress `testIsolation: true` (the project default, not overridden in `cypress.config.js`) clears cookies between every `it()`. `apiRequestWithSession` caches "session already set up" in `Cypress.env(sessionKey)` and skips re-setting the cookie on repeat calls — so a `describe` block using `before()` instead of `beforeEach()` for `setupApiAuthCookies()` will pass its first test then silently degrade to unauthenticated (401) on every subsequent test in that block. Always use `beforeEach()`, matching the pattern already used in `settings-api.cy.js` and `analytics-api.cy.js`. — Real backend bug found and fixed: `getNotifications()` in `lib/services/notification.js` called `.range(from, to)` unconditionally; when the requested page's offset exceeded the actual row count, Supabase/PostgREST returns `PGRST103 "Requested range not satisfiable"`, which the route re-threw as a generic 500 instead of a valid empty page. Fixed by doing a `head: true` count query first and short-circuiting to an empty `items: []` page when `from >= total`.
