...
**Last Updated:** 2026-06-14 (Race Reminder Settings API issue #199 — settings-api.cy.js 11/11 passing 100%; Running Tracker - Settings +1 feature +11 tests, now 7 features 36 tests) | 2026-06-14 (Racing Weight Profile API issue #270 — racing-weight-profile-api.cy.js 12/12 passing 100%; Running Tracker - Activities +1 feature +12 tests, now 20 features 345 tests) | 2026-06-14 (Friday Prep API issue #247 — friday-prep-api.cy.js 6/6 passing 100%; Running Tracker - Injury AI +1 feature +6 tests, now 4 features 30 tests) | 2026-06-13 (Satellite Map Fix issue #206 — satellite-map-fix.cy.js 13/13 passing 100%; Running Tracker - Activities +13 tests, now 333 total, 19 features) | 2026-06-10 (Cadence Chart Enhancements issue #166 — cadence-chart-enhancements-ui.cy.js 16/16 passing 100%; Running Tracker - Activities +16 tests, now 320 total, 18 features) | 2026-06-10 (HR Chart Enhancements issue #165 — hr-chart-enhancements-ui.cy.js 18/18 passing 100%, hr-zones-ai-insight-ui.cy.js 24/24 passing 100%; Running Tracker - Activities +42 tests, now 304 total, 17 features) | 2026-06-10 (Splits Bar Chart issue #164 — splits-bar-chart-ui.cy.js 24/24 passing 100%; Running Tracker - Activities +24 tests, now 262 total) | 2026-06-10 (Personal Bests Table issue #174 — personal-bests-api.cy.js 7/7 passing 100%, personal-bests-ui.cy.js 19/19 passing 100%; Running Tracker - Analytics +26 tests, now 124 total) | 2026-06-09 (Map Style Toggle issue #172 — map-style-toggle-ui.cy.js 14/14 passing 100%; Running Tracker - Activities +14 tests) | 2026-06-09 (Lazy Compute Derived Metrics issue #168 — lazy-compute-metrics-api.cy.js 5/5 active passing (2 pending, data-dependent), 100% active; Running Tracker - Activities +7 tests) | 2026-06-06 (Injury AI Roles issue #160 — injury-coach-api.cy.js 6/6, injury-coach-ui.cy.js 12/12, symptom-log-api.cy.js 6/6 — all 24/24 passing 100%; NEW module Running Tracker - Injury AI) | 2026-06-06 (Web Push Notifications issue #135 — settings-ui.cy.js +4 tests 21/21 passing, push-subscription-api.cy.js new 4/4 passing 100%; push toggle enable/disable/permission-denied/persist flows + API contract) | 2026-06-05 (Next Race Widget issue #153 — dashboard-ui-extended.cy.js 14/14 passing 100%; NextRace widget updated with title, description, edit-from-activity support) | 2026-06-05 (VO2max Target Effort issue #137 — 3 new spec files, 54/54 passing 100%; +19 upcoming-race target time tests, +35 analytics target effort tests)
**App Version:** 1.19



> **Note (2026-06-14 v1.19 Race Reminder Settings API issue #199 +11 tests):** settings-api.cy.js 11/11 passing (100%). NEW spec file under `cypress/e2e/running/settings/`. 3 describe blocks: A — auth guard (2 tests): GET/PATCH without session → 401 with error field. B — GET response shape (4 tests): authenticated returns 200 + data object + message:'OK'; all notification toggle fields present (notify_post_activity/notify_weekly_review/notify_friday_prep/notify_anomaly/notify_race_reminder); notify_race_reminder is a boolean; hr_zones_method is valid enum value. C — PATCH notify_race_reminder (5 tests): set to false → 200 with data.notify_race_reminder=false; set to true → 200 with data.notify_race_reminder=true; non-boolean value → 422 with issues; invalid hr_zones_method → 422 with issues; valid PATCH → 200 with message:'Settings updated'. Running Tracker - Settings: 7 features, 36 tests. Total: 198 features, 2,563 tests.

> **Note (2026-06-14 v1.19 Racing Weight Profile API issue #270 +12 tests):** racing-weight-profile-api.cy.js 12/12 passing (100%). NEW spec file under `cypress/e2e/running/activities/`. 3 describe blocks: A — auth guard (2 tests): GET without session → 401 with error field; PATCH without session → 401 with error field. B — GET response shape (3 tests): authenticated returns 200 with data object and message:'OK'; data contains required fields (id/email/height_cm/weight_kg/max_hr); weight_kg and height_cm are numbers when set. C — PATCH validation (7 tests): empty body → 400; negative weight_kg → 422 with issues; height_cm=0 → 422; max_hr>250 → 422; invalid sex → 422; invalid JSON → 400/422; valid PATCH with display_name → 200 with updated data. Running Tracker - Activities: 20 features, 345 tests. Total: 197 features, 2,552 tests.

> **Note (2026-06-14 v1.19 Friday Prep API issue #247 +6 tests):** friday-prep-api.cy.js 6/6 passing (100%). NEW spec file under `cypress/e2e/running/ai/`. 3 describe blocks: A — auth guard (1 test): GET `?type=friday_prep` without session → 401 with error field. B — type filter (2 tests): authenticated returns 200 + data array; every returned insight has `insight_type=friday_prep` when data exists. C — response shape (3 tests): each insight has required top-level fields (id/insight_type/status/content/data_refs/created_at); `data_refs.week_start` exists and matches `YYYY-MM-DD` format; insights are ordered by `created_at` descending. Running Tracker - Injury AI: 4 features, 30 tests. Total: 196 features, 2,540 tests.

> **Note (2026-06-13 v1.19 Satellite Map Fix issue #206 +13 tests):** satellite-map-fix.cy.js 13/13 passing (100%). NEW spec file under `cypress/e2e/running/activities/`. 4 describe blocks: Suite A — tile URL regression (3 tests): satellite toggle visible when polyline present; Satellite click does NOT trigger stadiamaps.com requests; Map style does NOT trigger ESRI tile requests on initial load. Suite B — toggle state (4 tests): Map button `aria-pressed=true` default; Satellite click flips states; Map click restores states; Leaflet map canvas exists after satellite switch (existence not visibility — Leaflet container clipped by `overflow:hidden` parents). Suite C — expanded modal satellite view (3 tests): modal opens with satellite toggle active; modal `[aria-label="Route map"]` exists; Escape closes modal after satellite switch. Suite D — no GPS polyline (3 tests): toggle absent, expand button absent, `[aria-label="Route map"]` absent (MediaCarousel returns null when polyline=null and photos=empty). Two fixes during run: (1) map canvas assertion `.should('be.visible')` → `.should('exist')` due to Leaflet overflow:hidden clipping; (2) "No GPS data" text assertion replaced with `[aria-label="Route map"] should not exist` because MediaCarousel never mounts RouteMap when polyline is null. Running Tracker - Activities: 19 features, 333 tests. Total: 195 features, 2,534 tests.
...
## Last Execution Results (2026-06-14 Focused Run: Race Reminder Settings API #199)

| Group                                                                        | Spec Files | Tests | Passed | Failed | Pending | Status   |
| ---------------------------------------------------------------------------- | ---------- | ----- | ------ | ------ | ------- | -------- |
| running/settings (Settings API) — issue #199                                 | 1          | 11    | 11     | 0      | 0       | ✅       |
| **Total**                                                                    | **1**      | **11** | **11** | **0**  | **0**   | **100%** |

**Status:** 11/11 passing (100%). settings-api.cy.js: Section A — auth guard (2): GET/PATCH without session → 401. Section B — GET shape (4): authenticated 200 + data + message:'OK'; notify_post_activity/notify_weekly_review/notify_friday_prep/notify_anomaly/notify_race_reminder all present; notify_race_reminder is boolean; hr_zones_method is valid enum. Section C — PATCH notify_race_reminder (5): false → 200 + data.notify_race_reminder=false; true → 200 + data.notify_race_reminder=true; non-boolean → 422; invalid hr_zones_method → 422; valid PATCH → message:'Settings updated'.

### Previous Execution Results (2026-06-14 Focused Run: Racing Weight Profile API #270)

| Group                                                                        | Spec Files | Tests | Passed | Failed | Pending | Status   |
| ---------------------------------------------------------------------------- | ---------- | ----- | ------ | ------ | ------- | -------- |
| running/activities (Racing Weight Profile API) — issue #270                  | 1          | 12    | 12     | 0      | 0       | ✅       |
| **Total**                                                                    | **1**      | **12** | **12** | **0**  | **0**   | **100%** |

**Status:** 12/12 passing (100%). racing-weight-profile-api.cy.js: Section A — auth guard (2): GET/PATCH without session → 401. Section B — GET shape (3): authenticated 200 + data object + message:'OK'; required fields present (id/email/height_cm/weight_kg/max_hr); numeric types validated. Section C — PATCH validation (7): empty body → 400; negative weight_kg → 422; height_cm=0 → 422; max_hr>250 → 422; invalid sex → 422; invalid JSON → 400/422; valid display_name PATCH → 200 with updated data.

### Previous Execution Results (2026-06-10 Focused Run: Personal Bests Table #174)
...
| 108 | cypress/e2e/running/activities/satellite-map-fix.cy.js | Satellite Map Fix UI (issue #206) | 13       | Suite A — Tile URL regression (3): satellite toggle present with polyline, Satellite click triggers no stadiamaps.com requests, Map style triggers no ESRI tile requests on initial load. Suite B — Toggle state (4): Map aria-pressed=true default, Satellite flips states, Map restores, Leaflet canvas exists after switch. Suite C — Expanded modal satellite view (3): modal opens with satellite active, modal Route map canvas exists, Escape closes modal. Suite D — No GPS polyline fallback (3): toggle absent, expand button absent, Route map absent when MediaCarousel returns null. Fix: existence not visibility for Leaflet canvas (overflow:hidden clipping); Route map absent replaces "No GPS data" text (MediaCarousel never mounts RouteMap). |

**Total Automated Test Cases: 2,534** (added 14 Map Style Toggle tests 2026-06-09 — issue #172; previous total: 2,316) (added 7 Lazy Compute Metrics tests 2026-06-09 — issue #168; previous total: 2,309) (added 54 VO2max Target Effort tests 2026-06-05 — issue #137; previous total: 2,223) (added 17 Settings page tests 2026-06-04 — issue #132; previous total: 2,206) (added 17 AI Coach page tests 2026-06-04; previous total: 2,189) (added 19 compare-runs tests 2026-06-03; previous total: 2,170) (added 56 upcoming races tests 2026-06-02; previous total: 2,114) (added 34 analytics tests 2026-05-31; previous total: 2,044) (added 13 race-log search+filter tests 2026-05-29; previous total: 2,031)
