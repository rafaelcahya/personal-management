...
| Running Tracker - Analytics     | 9              | 9         | 0           | 0          | 100%       | 137        |
...
> **Note (2026-06-10 v1.8 Qualifying Run Count issue #170 +13 tests):** qualifying-run-count-ui.cy.js 13/13 passing (100%). NEW spec file under `cypress/e2e/running/analytics/`. 6 describe blocks. Current VO2max (data exists, 3 tests): `#vo2maxSampleSize_analyticsPage` renders; "Based on 12 qualifying runs (last 30 days)"; singular "run" when count=1. Current VO2max (empty, 2 tests): `#vo2maxEmptyCount_analyticsPage` shows count + minimum threshold. VO2max Trend (data exists, 2 tests): `#vo2maxTrendDataCount_analyticsPage` renders + "Showing 5 data points from last 90 activities". VO2max Trend (empty, 2 tests): `#vo2maxTrendEmptyCount_analyticsPage` shows count + criteria. EF Trend (data exists, 2 tests): `#efTrendDataCount_analyticsPage` renders + correct count. EF Trend (empty, 2 tests): `#efTrendEmptyCount_analyticsPage` shows count + criteria. analytics-ui.cy.js updated (2 assertions): empty state text changed to new issue #170 copy. 6 new IDs in `test-ids.js` + `app-constants.json`. Implementation: `CurrentVo2maxStat.jsx` — sample_size shown as subtext (data) and inline count (empty). `Vo2maxTrendChart.jsx` + `EfTrendChart.jsx` — empty state switched from EmptyState component to inline div with ID on count; data subtext got ID + new "Showing X data points" format. Running Tracker - Analytics: 9 features, 137 tests. Total: 195 features, 2,534 tests.
