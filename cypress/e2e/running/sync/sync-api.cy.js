...
// Covers: Running Tracker Strava sync API endpoints
//   POST /api/running/v1/sync/strava       — trigger backfill, auth guard
//   GET  /api/running/v1/sync/status       — connection status, auth guard
//   GET  /api/running/v1/auth/strava/callback — public OAuth redirect (no code / invalid code)

import endpoints from '../../../fixtures/app-constants.json'

const syncEndpoints = endpoints.endpoints.running_sync

// ---------------------------------------------------------------------------
// A. POST /api/running/v1/sync/strava (authenticated)
// ---------------------------------------------------------------------------
...
describe('Sync API — POST /api/running/v1/sync/strava (authenticated)', () => {
  ...
  // Happy path — authenticated POST triggers Inngest backfill job
  ...
  // Shape check — success is boolean, message is string
...
// ---------------------------------------------------------------------------
// B. GET /api/running/v1/sync/status (authenticated)
// ---------------------------------------------------------------------------
...
describe('Sync API — GET /api/running/v1/sync/status (authenticated)', () => {
  ...
  // Happy path — test user has no Strava credentials row → connected: false
  ...
  // Shape check — connected is boolean, last_sync_at is string or null
...
// ---------------------------------------------------------------------------
// C. GET /api/running/v1/auth/strava/callback (public OAuth redirect)
// ---------------------------------------------------------------------------
...
describe('Sync API — GET /api/running/v1/auth/strava/callback (OAuth redirect)', () => {
  ...
  // Missing code param → redirect to settings error URL (no token exchange attempted)
  ...
  // Invalid code → Strava token exchange fails → redirect to settings error URL
...
// ---------------------------------------------------------------------------
// D. Unauthenticated access — isolated describe, no beforeEach auth
// ---------------------------------------------------------------------------
