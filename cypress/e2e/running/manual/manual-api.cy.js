...
// Covers: Running Tracker Manual Entry API endpoints
//   POST /api/running/v1/activities         — create activity, dedup guard
//   GET  /api/running/v1/activities         — list with filters + invalid params
//   GET  /api/running/v1/activities/:id     — detail + splits
//   PATCH /api/running/v1/activities/:id    — edit notes/type/RPE + no-op empty body (WB-1)
//   DELETE /api/running/v1/activities/:id   — cascade delete
//   POST /api/running/v1/health/subjective  — upsert semantics (200 always)
//   GET  /api/running/v1/health/subjective  — paginated list
//   PATCH /api/running/v1/health/subjective/:id
//   DELETE /api/running/v1/health/subjective/:id
//   POST /api/running/v1/health/weight      — create weight log (201)
//   GET  /api/running/v1/health/weight      — paginated list
//   PATCH /api/running/v1/health/weight/:id
//   DELETE /api/running/v1/health/weight/:id

import constants from '../../../fixtures/app-constants.json'

const ep = constants.endpoints.running_manual
...
describe('Manual API — Activities CRUD (authenticated)', () => {
  ...
  // A-1: POST with all required fields → 201, response has data.id
  ...
  // A-2: POST → GET list → newly created activity appears in list
  ...
  // A-3: GET list with ?type=easy → only easy activities in results
  ...
  // A-4: GET list with ?page=abc (invalid) → 200, falls back to page 1 (CB-3 fix)
  ...
  // A-5: GET /:id → 200 with activity and splits keys
  ...
  // A-6: GET /:id for non-existent UUID → 404
  ...
  // A-7: PATCH /:id with { notes } → 200, updated notes in response
  ...
  // A-8: PATCH /:id with empty body {} → 422 (schema refine rejects empty patch)
  ...
  // A-11: POST same timestamp twice → 409 with existing.started_at (S-2 fix)
...
describe('Manual API — Subjective Health CRUD (authenticated)', () => {
  ...
  // B-1: POST log_date entry → 200 (upsert semantics, CB-2 fix)
  ...
  // B-2: POST same log_date again → 200, row updated (upsert replaces, not inserts)
  ...
  // B-3: GET list → 200 with data array and total
  ...
  // B-4 / B-5 / B-6: PATCH mood, DELETE, verify gone from list
...
describe('Manual API — Weight Log CRUD (authenticated)', () => {
  ...
  // C-1: POST weight entry → 201 with data.id
  ...
  // C-2: GET list → 200 with data array and total
  it('GET list → 200 with data array and total', () => {
  ...
  // C-3 / C-4: PATCH notes → 200 updated; DELETE → 204
