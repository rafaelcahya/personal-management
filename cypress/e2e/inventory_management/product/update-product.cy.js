...
describe('PATCH Update Product API - /api/inventory/v1/product/update/[id]', () => {
  ...
  describe('Authentication', () => {
    it('should return 200 for authenticated user with valid request', () => {
      createProductWithStock(10).then((product) => {
        cy.UpdateProduct(product.id, buildUpdateRequest()).then((response) => {
    ...
    it('should return 307 or 401 without authentication', () => {
      cy.clearApiAuth()
      cy.UpdateProductNoAuth(productWithStock.id, buildUpdateRequest()).then((response) => {
  ...
  describe('Required Field Validation', () => {
    it('should return 400 when usage_quantity is missing', () => {
      cy.UpdateProduct(productWithStock.id, buildUpdateRequest({ usage_quantity: undefined })).then(
        (response) => {
          expect(response.status).to.eq(400)
          expect(response.body.success).to.be.false
          expect(response.body.error).to.be.an('array')
          expect(response.body.error).to.include('usage quantity is required')
        }
      )
    })
    ...
    it('should return 400 when start_usage_date is missing', () => {
      cy.UpdateProduct(
        ...
        expect(response.body.success).to.be.false
        expect(response.body.error).to.be.an('array')
        expect(response.body.error).to.include('start usage date is required')
      })
    })
  ...
  describe('Usage Quantity Validation', () => {
    it('should return 400 when usage_quantity is non-numeric', () => {
      cy.UpdateProduct(productWithStock.id, buildUpdateRequest({ usage_quantity: 'abc' })).then(
        (response) => {
          expect(response.status).to.eq(400)
          expect(response.body.success).to.be.false
          expect(response.body.error).to.include('Usage quantity must be a valid number')
        }
      )
    })

    it('should return 400 when usage_quantity is negative', () => {
      cy.UpdateProduct(productWithStock.id, buildUpdateRequest({ usage_quantity: -1 })).then(
        (response) => {
          expect(response.status).to.eq(400)
          expect(response.body.success).to.be.false
          expect(response.body.error).to.include('Usage quantity cannot be negative')
        }
      )
    })

    it('should return 400 when usage_quantity is zero', () => {
      cy.UpdateProduct(productWithStock.id, buildUpdateRequest({ usage_quantity: 0 })).then(
        (response) => {
          expect(response.status).to.eq(400)
          expect(response.body.success).to.be.false
          expect(response.body.error[0]).to.include('Usage quantity must be greater than 0')
        }
      )
    })

    it('should return 400 when usage_quantity is float', () => {
      cy.UpdateProduct(productWithStock.id, buildUpdateRequest({ usage_quantity: 1.5 })).then(
        (response) => {
          expect(response.status).to.eq(400)
          expect(response.body.success).to.be.false
          expect(response.body.error).to.include('Usage quantity must be a whole number')
        }
      )
    })
  })
  ...
  describe('Date Validation', () => {
    it('should return 400 when start_usage_date is invalid format', () => {
      cy.UpdateProduct(
        ...
        expect(response.status).to.eq(400)
        expect(response.body.success).to.be.false
        expect(response.body.error).to.include('Start usage date must be valid ISO date')
      })
    })
    ...
    it('should accept valid date string format', () => {
      createProductWithStock(10).then((product) => {
        cy.UpdateProduct(product.id, buildUpdateRequest({ start_usage_date: '2026-03-20' })).then(
          (response) => {
            expect(response.status).to.eq(200)
            expect(response.body.success).to.be.true
          }
  ...
  describe('Not Found & Business Logic Error', () => {
    it('should return 404 when product id does not exist', () => {
      cy.UpdateProduct(999999999, buildUpdateRequest()).then((response) => {
    ...
    it('should return 422 when usage_quantity exceeds available stock', () => {
      createProductWithStock(2).then((product) => {
        cy.UpdateProduct(product.id, buildUpdateRequest({ usage_quantity: 999 })).then(
          (response) => {
            expect(response.status).to.eq(422)
            expect(response.body.success).to.be.false
            expect(response.body.error).to.include('Insufficient stock')
          }
        )
      })
    })
  ...
  describe('Response Structure', () => {
    it('should return correct top-level keys', () => {
      createProductWithStock(10).then((product) => {
        cy.UpdateProduct(product.id, buildUpdateRequest()).then((response) => {
          expect(response.body).to.have.all.keys('success', 'product', 'message')
          expect(response.body.success).to.be.true
    ...
    it('should return correct success message', () => {
      createProductWithStock(10).then((product) => {
        cy.UpdateProduct(product.id, buildUpdateRequest()).then((response) => {
          expect(response.body.message).to.eq('Product activated')
        })
      })
    })
  ...
  describe('Update Logic', () => {
    it('product_status should be active after update', () => {
      createProductWithStock(10).then((product) => {
        cy.UpdateProduct(product.id, buildUpdateRequest()).then((response) => {
          expect(response.body.product.product_status).to.eq('active')
        })
      })
    })
    ...
    it('quantity should decrease by usage_quantity', () => {
      ...
      createProductWithStock(initialQuantity).then((product) => {
        cy.UpdateProduct(product.id, buildUpdateRequest({ usage_quantity: usageQty })).then(
          (response) => {
            expect(response.body.product.quantity).to.eq(initialQuantity - usageQty)
          }
        )
      })
    })
    ...
    it('usage_quantity in response should increase by amount used', () => {
      createProductWithStock(10).then((product) => {
        ...
        cy.UpdateProduct(product.id, buildUpdateRequest({ usage_quantity: usageQty })).then(
          (response) => {
            expect(response.body.product.usage_quantity).to.eq(initialUsageQty + usageQty)
          }
        )
      })
    })
    ...
    it('usage_date should be set to start_usage_date from request', () => {
      ...
      createProductWithStock(10).then((product) => {
        cy.UpdateProduct(product.id, buildUpdateRequest({ start_usage_date: usageDate })).then(
          (response) => {
            const responseDate = new Date(response.body.product.usage_date).toISOString()
    ...
    it('note should be updated when provided', () => {
      ...
      createProductWithStock(10).then((product) => {
        cy.UpdateProduct(product.id, buildUpdateRequest({ note: newNote })).then((response) => {
  ...
  describe('API vs Database Comparison', () => {
    it('DB quantity should match API response quantity after update', () => {
      ...
      createProductWithStock(10).then((product) => {
        cy.UpdateProduct(product.id, buildUpdateRequest({ usage_quantity: usageQty }))
          .then((response) => response.body.product)
          .then((apiProduct) => {
            cy.getProductWithQuantityFromDb(product.id).then((dbProduct) => {
              expect(dbProduct).to.not.be.null
              expect(dbProduct.quantity).to.eq(apiProduct.quantity)
              expect(dbProduct.usage_quantity).to.eq(apiProduct.usage_quantity)
              expect(dbProduct.product_status).to.eq(apiProduct.product_status)
      ...
      createProductWithStock(initialStock).then((product) => {
        ...
        cy.UpdateProduct(product.id, buildUpdateRequest({ usage_quantity: usageQty })).then(
          (res) => {
            ...
            cy.getLatestProductHistoryFromDb(product.id).then((history) => {
              expect(history).to.not.be.null
              expect(Number(history.depleted_quantity)).to.eq(usageQty)
              expect(Number(history.remaining_quantity)).to.eq(initialStock - usageQty)
              expect(Number(history.quantity)).to.eq(initialStock)
    ...
    it('history remaining_quantity + depleted_quantity should equal quantity', () => {
      createProductWithStock(10).then((product) => {
        cy.UpdateProduct(product.id, buildUpdateRequest({ usage_quantity: 3 })).then((res) => {
          expect(res.status).to.eq(200)
          ...
          cy.getLatestProductHistoryFromDb(product.id).then((history) => {
            ...
            const remaining = Number(history.remaining_quantity)
    ...
    it('history remaining_quantity should match product quantity in DB after update', () => {
      createProductWithStock(10).then((product) => {
        cy.UpdateProduct(product.id, buildUpdateRequest({ usage_quantity: 3 })).then((res) => {
          ...
          cy.getLatestProductHistoryFromDb(product.id).then((history) => {
            cy.getProductWithQuantityFromDb(product.id).then((dbProduct) => {
              expect(Number(history.remaining_quantity)).to.eq(dbProduct.quantity)
            })
          })
        })
      })
    ...
    it('history product_status should be active after update', () => {
      createProductWithStock(10).then((product) => {
        cy.UpdateProduct(product.id, buildUpdateRequest({ usage_quantity: 1 })).then((res) => {
          expect(res.status).to.eq(200)
    ...
    it('multiple updates should each create separate history records', () => {
      createProductWithStock(20).then((product) => {
        ...
        cy.UpdateProduct(product.id, buildUpdateRequest({ usage_quantity: 2 })).then((res) => {
          expect(res.status).to.eq(200)
        })
