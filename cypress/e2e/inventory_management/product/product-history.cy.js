...
describe("GET Product Stock History API - /api/inventory/v1/product/stock/history/[id]", () => {
            ...
            cy.AddProductName({
                ...
                cy.AddProduct({
                    ...
                    cy.CreateProductStock(
                        buildStockRequest({ quantity_added: 5, price: 10000 }),
                    );
                    cy.CreateProductStock(
                        buildStockRequest({ quantity_added: 3, price: 20000 }),
                    );
                    cy.CreateProductStock(
                        buildStockRequest({ quantity_added: 2, price: 15000 }),
                    );
                });
            });
        });
    ...
    describe("Authentication", () => {
        ...
        it("should return 307 or 401 without authentication", () => {
            cy.clearApiAuth();
            cy.GetProductStockHistoryNoAuth(validProductListId).then(
                (response) => {
    ...
    describe("ID Validation", () => {
        it("should return 400 when id is non-numeric", () => {
            cy.GetProductStockHistory("abc").then((response) => {
                ...
                expect(response.body.error).to.eq(
                    "Product list ID must be a valid number",
                );
            });
        });
        ...
        it("should return 400 when id is float", () => {
            cy.GetProductStockHistory("1.5").then((response) => {
                ...
                expect(response.body.success).to.be.false;
                expect(response.body.error).to.eq(
                    "Product list ID must be an integer",
                );
            });
        });
    ...
    describe("Data Accuracy", () => {
        ...
        it("should return history only for authenticated user", () => {
            cy.getTestUserId().then((userId) => {
                cy.GetProductStockHistory(validProductListId).then(
                    (response) => {
                        response.body.history.forEach((item) => {
            ...
            cy.GetProductStockHistory(validProductListId).then((response) => {
                ...
                for (let i = 0; i < history.length - 1; i++) {
                    const current = new Date(
                        history[i].purchase_date,
                    ).getTime();
                    const next = new Date(
                        history[i + 1].purchase_date,
                    ).getTime();
        ...
        it("newly created stock should appear in history", () => {
            ...
            cy.CreateProductStock(
                buildStockRequest({ quantity_added: qty, price }),
            ).then((createRes) => {
                ...
                cy.GetProductStockHistory(validProductListId).then(
                    (response) => {
                        const found = response.body.history.find(
                            (h) => h.id === stockId,
                        );
    ...
    describe("Data Types", () => {
        ...
        it("should return valid ISO timestamps for created_at", () => {
            cy.GetProductStockHistory(validProductListId).then((response) => {
                ...
                expect(new Date(item.created_at).toString()).to.not.eq(
                    "Invalid Date",
                );
            });
        });
        ...
        it("should return valid date for purchase_date", () => {
            cy.GetProductStockHistory(validProductListId).then((response) => {
                ...
                expect(new Date(item.purchase_date).toString()).to.not.eq(
                    "Invalid Date",
                );
            });
        });
    ...
    describe("API vs Database Comparison", () => {
        ...
        it("history items should match DB records", () => {
            cy.GetProductStockHistory(validProductListId).then((response) => {
                ...
                cy.getProductQuantityHistoryFromDb(validProductListId).then(
                    (dbHistory) => {
                        expect(apiHistory.length).to.eq(dbHistory.length);

                        apiHistory.forEach((apiItem) => {
                            const dbItem = dbHistory.find(
                                (d) => d.id === apiItem.id,
                            );
                            expect(dbItem).to.not.be.undefined;
                            expect(apiItem.quantity_added).to.eq(
                                dbItem.quantity_added,
                            );
                            expect(Number(apiItem.price)).to.eq(
                                Number(dbItem.price),
                            );
                            expect(apiItem.product_list_id).to.eq(
                                dbItem.product_list_id,
                            );
                        });
                    },
                );
        ...
        it("history count should increase by 1 after adding stock", () => {
            cy.GetProductStockHistory(validProductListId).then(
                (beforeResponse) => {
                    ...
                    cy.GetProductStockHistory(validProductListId).then(
                        (afterResponse) => {
                            expect(afterResponse.body.history.length).to.eq(
                                beforeCount + 1,
                            );
                        },
                    );
