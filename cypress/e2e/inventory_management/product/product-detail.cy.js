...
describe("GET Product By ID API - /api/inventory/v1/product/[id]", () => {
    ...
    describe("Authentication", () => {
        ...
        it("should return 307 or 401 without authentication", () => {
            cy.clearApiAuth();
            cy.then(() => {
                cy.GetProductDetailNoAuth(createdProduct.id).then(
                    (response) => {
    ...
    describe("Not Found", () => {
        ...
        it("should return 404 when product belongs to another user", () => {
            cy.intercept("GET", "/api/inventory/v1/product/888888888", {
                statusCode: 404,
    ...
    describe("Data Accuracy", () => {
        it("should return correct data matching created product", () => {
            cy.then(() => {
                cy.GetProductDetail(createdProduct.id).then((response) => {
                    const product = response.body.data;
                    ...
                    expect(product.usage_quantity).to.eq(
                        createdProduct.usage_quantity,
                    );
                });
            });
        });

        it("should return enriched product name string", () => {
            cy.then(() => {
                cy.GetProductDetail(createdProduct.id).then((response) => {
                    const product = response.body.data;
        ...
        it("should return enriched brand name string", () => {
            cy.then(() => {
                cy.GetProductDetail(createdProduct.id).then((response) => {
                    const product = response.body.data;
    ...
    describe("Default Field Values", () => {
        it("should have product_status inactive on creation", () => {
            cy.then(() => {
                cy.GetProductDetail(createdProduct.id).then((response) => {
        ...
        it("should have quantity 0 on creation", () => {
            cy.then(() => {
                cy.GetProductDetail(createdProduct.id).then((response) => {
        ...
        it("should have is_favorite false on creation", () => {
            cy.then(() => {
                cy.GetProductDetail(createdProduct.id).then((response) => {
        ...
        it("should have usage_date null on creation", () => {
            cy.then(() => {
                cy.GetProductDetail(createdProduct.id).then((response) => {
        ...
        it("should have deleted_at null on creation", () => {
            cy.then(() => {
                cy.GetProductDetail(createdProduct.id).then((response) => {
    ...
    describe("Data Types", () => {
        ...
        it("should return valid ISO timestamps", () => {
            cy.then(() => {
                cy.GetProductDetail(createdProduct.id).then((response) => {
                    const product = response.body.data;
                    expect(new Date(product.created_at).toString()).to.not.eq(
                        "Invalid Date",
                    );
                    expect(new Date(product.updated_at).toString()).to.not.eq(
                        "Invalid Date",
                    );
                });
            });
        ...
        it("should return valid product_status value", () => {
            cy.then(() => {
                cy.GetProductDetail(createdProduct.id).then((response) => {
                    expect(response.body.data.product_status).to.be.oneOf([
                        "active",
                        "inactive",
                    ]);
                });
    ...
    describe("API vs Database Comparison", () => {
        it("should match all fields with database record", () => {
            cy.then(() => {
                cy.GetProductDetail(createdProduct.id).then((response) => {
                    const apiProduct = response.body.data;

                    cy.getSingleProductFromDb(createdProduct.id).then(
                        (dbProduct) => {
