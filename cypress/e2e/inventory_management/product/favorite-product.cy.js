    ...
    describe("Body Validation", () => {
        it("should return 400 when isFavorite is missing", () => {
            createFreshProduct().then((product) => {
                cy.apiRequestWithSession(
                    "PATCH",
                    `/api/inventory/v1/product/${product.id}/favorite`,
                    { body: {} },
                ).then((response) => {
                    expect(response.status).to.eq(400);
                    expect(response.body.success).to.be.false;
        ...
        it("should return 400 when body is invalid JSON", () => {
            createFreshProduct().then((product) => {
                cy.request({
                    method: "PATCH",
                    url: `/api/inventory/v1/product/${product.id}/favorite`,
                    body: "invalid-json",
    ...
    describe("API vs Database Comparison", () => {
        it("is_favorite in DB should be true after API call with true", () => {
            createFreshProduct().then((product) => {
                ...
                cy.getProductFavoriteStatusFromDb(product.id).then(
                    (dbProduct) => {
                ...
                .then((beforeCount) => {
                    createFreshProduct().then((product) => {
                        ...
                        cy.GetProductSummary().then((response) => {
                            expect(response.body.data.favoriteProducts).to.eq(
                                beforeCount + 1,
                            );
                        });
                    });
