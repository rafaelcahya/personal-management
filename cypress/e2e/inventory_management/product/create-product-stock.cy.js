...
describe("POST Create Product Stock API - /api/inventory/v1/product/stock/create", () => {
    ...
    describe("Required Field Validation", () => {
        ["product_list_id", "quantity_added", "price", "purchase_date"].forEach(
            (field) => {
                ...
                it(`should return 400 when ${field} is null`, () => {
                    cy.CreateProductStock(buildRequest({ [field]: null })).then(
                        (response) => {
                            expect(response.status).to.eq(400);
                            expect(response.body.error).to.be.an("array");
                        },
                ...
                it(`should return 400 when ${field} is empty string`, () => {
                    cy.CreateProductStock(buildRequest({ [field]: "" })).then(
                        (response) => {
                            expect(response.status).to.eq(400);
                            expect(response.body.error).to.be.an("array");
                        },
    ...
    describe("product_list_id Validation", () => {
        it("should return 400 when product_list_id is non-numeric", () => {
            cy.CreateProductStock(
                buildRequest({ product_list_id: "abc" }),
            ).then((response) => {
                expect(response.status).to.eq(400);
                expect(response.body.error).to.include(
                    "Product list id must be a positive integer",
                );
            });
        });

        it("should return 400 when product_list_id is float", () => {
            cy.CreateProductStock(buildRequest({ product_list_id: 1.5 })).then(
                (response) => {
                    expect(response.status).to.eq(400);
                    expect(response.body.error).to.include(
                        "Product list id must be a positive integer",
        ...
        it("should return 400 when product_list_id is zero", () => {
            cy.CreateProductStock(buildRequest({ product_list_id: 0 })).then(
                (response) => {
                    expect(response.status).to.eq(400);
                    expect(response.body.error).to.include(
                        "Product list id must be a positive integer",
        ...
        it("should return 400 when product_list_id is negative", () => {
            cy.CreateProductStock(buildRequest({ product_list_id: -1 })).then(
                (response) => {
                    expect(response.status).to.eq(400);
                    expect(response.body.error).to.include(
                        "Product list id must be a positive integer",
        ...
        it("should return 404 when product_list_id does not exist", () => {
            cy.CreateProductStock(
                buildRequest({ product_list_id: 999999999 }),
            ).then((response) => {
    ...
    describe("quantity_added Validation", () => {
        it("should return 400 when quantity_added is non-numeric", () => {
            cy.CreateProductStock(buildRequest({ quantity_added: "abc" })).then(
                (response) => {
                    expect(response.status).to.eq(400);
                    expect(response.body.error).to.include(
        ...
        it("should return 400 when quantity_added is float", () => {
            cy.CreateProductStock(buildRequest({ quantity_added: 1.5 })).then(
                (response) => {
                    expect(response.status).to.eq(400);
                    expect(response.body.error).to.include(
                        "Quantity added must be a whole number",
                    );
                },
            );
        });
        ...
        it("should return 400 when quantity_added is zero", () => {
            cy.CreateProductStock(buildRequest({ quantity_added: 0 })).then(
                (response) => {
                    expect(response.status).to.eq(400);
                    expect(response.body.error).to.include(
        ...
        it("should return 400 when quantity_added is negative", () => {
            cy.CreateProductStock(buildRequest({ quantity_added: -1 })).then(
                (response) => {
                    expect(response.status).to.eq(400);
                    expect(response.body.error).to.include(
                        "Quantity added must be greater than 0",
    ...
    describe("price Validation", () => {
        it("should return 400 when price is non-numeric", () => {
            cy.CreateProductStock(buildRequest({ price: "abc" })).then(
                (response) => {
                    expect(response.status).to.eq(400);
                    expect(response.body.error).to.include(
        ...
        it("should return 400 when price is negative", () => {
            cy.CreateProductStock(buildRequest({ price: -1 })).then(
                (response) => {
                    expect(response.status).to.eq(400);
                    expect(response.body.error).to.include(
        ...
        it("should accept price of 0 (free item)", () => {
            cy.CreateProductStock(buildRequest({ price: 0 })).then(
                (response) => {
                    expect(response.status).to.eq(201);
                    expect(response.body.success).to.be.true;
                },
        ...
        it("should accept valid decimal price", () => {
            cy.CreateProductStock(buildRequest({ price: 15000.5 })).then(
                (response) => {
                    expect(response.status).to.eq(201);
                    expect(response.body.success).to.be.true;
                },
    ...
    describe("purchase_date Validation", () => {
        it("should return 400 when purchase_date is invalid format", () => {
            cy.CreateProductStock(
                buildRequest({ purchase_date: "not-a-date" }),
            ).then((response) => {
                expect(response.status).to.eq(400);
                expect(response.body.error).to.include(
        ...
        it("should accept valid ISO date format", () => {
            cy.CreateProductStock(
                buildRequest({ purchase_date: "2026-03-20T00:00:00.000Z" }),
            ).then((response) => {
                expect(response.status).to.eq(201);
                expect(response.body.success).to.be.true;
            });
        ...
        it("should accept valid date string format", () => {
            cy.CreateProductStock(
                buildRequest({ purchase_date: "2026-03-20" }),
            ).then((response) => {
                expect(response.status).to.eq(201);
                expect(response.body.success).to.be.true;
            });
    ...
    describe("Create Logic", () => {
        it("should save correct quantity_added in response", () => {
            const qty = 5;
            cy.CreateProductStock(buildRequest({ quantity_added: qty })).then(
                (response) => {
        ...
        it("should save correct purchase_date in response", () => {
            ...
            cy.CreateProductStock(buildRequest({ purchase_date: date })).then(
                (response) => {
                    const responseDate = response.body.data.purchase_date.slice(
                        0,
                        10,
                    );
        ...
        it("should save correct product_list_id in response", () => {
            cy.CreateProductStock(buildRequest()).then((response) => {
                expect(response.body.data.product_list_id).to.eq(
                    validProductListId,
                );
            });
        });
    ...
    describe("Product Quantity Update Logic", () => {
        it("should increase product quantity by quantity_added", () => {
            ...
            cy.getProductWithQuantityFromDb(validProductListId).then(
                (before) => {
                    const beforeQty = Number(before.quantity);

                    cy.CreateProductStock(
                        buildRequest({ quantity_added: qtyToAdd }),
                    ).then((res) => {
                        expect(res.status).to.eq(201);

                        cy.getProductWithQuantityFromDb(
                            validProductListId,
                        ).then((after) => {
                            expect(Number(after.quantity)).to.eq(
                                beforeQty + qtyToAdd,
                            );
                        });
                    });
        ...
        it("should correctly accumulate quantity after multiple stock additions", () => {
            ...
            cy.getProductWithQuantityFromDb(validProductListId).then(
                (before) => {
                    ...
                    cy.CreateProductStock(
                        buildRequest({ quantity_added: firstAdd }),
                    ).then((res) => {
                        expect(res.status).to.eq(201);
                    });

                    cy.CreateProductStock(
                        buildRequest({ quantity_added: secondAdd }),
                    ).then((res) => {
                        expect(res.status).to.eq(201);
                    });
    ...
    describe("API vs Database Comparison", () => {
        it("should create record in product_quantity table", () => {
            cy.CreateProductStock(buildRequest()).then((res) => {
                ...
                cy.getLatestProductQuantityFromDb(validProductListId).then(
                    (dbRecord) => {
        ...
        it("DB record should match API response data", () => {
            ...
            cy.CreateProductStock(
                buildRequest({ quantity_added: qty, price }),
            ).then((res) => {
                expect(res.status).to.eq(201);
                ...
                cy.getLatestProductQuantityFromDb(validProductListId).then(
                    (dbRecord) => {
                        ...
                        expect(dbRecord.quantity_added).to.eq(
                            apiData.quantity_added,
                        );
                        expect(Number(dbRecord.price)).to.eq(
                            Number(apiData.price),
                        );
                        expect(dbRecord.product_list_id).to.eq(
                            apiData.product_list_id,
                        );
        ...
        it("should create separate record for each stock addition", () => {
            cy.getProductQuantityCountFromDb(validProductListId).then(
                (beforeCount) => {
                    cy.CreateProductStock(buildRequest()).then((res) => {
                        expect(res.status).to.eq(201);
                    });
