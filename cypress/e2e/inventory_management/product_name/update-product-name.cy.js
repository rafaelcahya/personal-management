...
describe('Product Name Update API', () => {
  ...
  describe('Authentication & Authorization', () => {
    it('should update product name successfully (200)', () => {
      ...
      cy.UpdateProductName(testProductNameId, request).then((response) => {
  ...
  describe('Success Scenario', () => {
    it('should update product name successfully (200)', () => {
      ...
      cy.UpdateProductName(testProductNameId, request).then((response) => {
        ...
        const exactFields = ['product_name_status', 'note', 'product_name']
        exactFields.forEach((field) => {
          expect(response.body.productName[field]).to.eq(request[field])
        })
  ...
  describe('Data Integrity - API vs Database Comparison', () => {
    ...
    describe('Complete Field Comparison', () => {
      it('should match all fields between API and DB', function () {
        ...
        cy.getSingleProductNameFromDb(productNameId.toString()).then((rows) => {
          dbProductName = rows[0]
          cy.log('DB product name:', JSON.stringify(dbProductName))
  ...
  describe('Business Logic — product name is deleted', () => {
    it('should auto-set deleted_at when product name is deleted', () => {
      cy.UpdateProductName(testProductNameId, {
        ...
        expect(new Date(response.body.productName.deleted_at).toString()).to.not.eq('Invalid Date')
      })
    })
  ...
  describe('Request Body Validation', () => {
    ...
    it('should return 400 for invalid JSON string', () => {
      cy.UpdateProductName(testProductNameId, 'NULL').then((response) => {
        expect(response.status).to.eq(400)
        expect(response.body.success).to.be.false
        expect(response.body.error).to.eq('Invalid JSON in request body')
      })
    ...
    it('should return 400 when product name status is missing', () => {
      cy.UpdateProductName(testProductNameId, {
        ...
        expect(response.body.success).to.be.false
        expect(response.body.error).to.be.an('array')
        expect(response.body.error).to.include('product name status is required')
      })
    })
...
// Covers v1.14: uniqueness check (409 CONFLICT) on update endpoint — excludes current record.
// Isolated in its own top-level describe so the before() seed failure above does not cause skips.
describe('Product Name Update — Uniqueness Check: 409 Conflict', () => {
  // Use timestamp suffix to guarantee unique names that won't collide across test runs
  const ts = Date.now()
  const nameA = `UpdateConflict-A-${ts}`
  const nameB = `UpdateConflict-B-${ts}`
  ...
  before(() => {
    ...
    // Seed name A (this is the one we'll try to update to name B)
    cy.AddProductName({ product_name: nameA, product_name_status: 'active' }).then((res) => {
      expect(res.status).to.eq(201)
      idA = res.body.productName.id
    })

    // Seed name B (the "already existing" conflicting name)
  ...
  it('should return 409 when updating to a name already used by another record', () => {
    cy.UpdateProductName(idA, {
      product_name: nameB,
      product_name_status: 'active',
    }).then((res) => {
  ...
  it('should return 409 for duplicate name regardless of case (case-insensitive)', () => {
    cy.UpdateProductName(idA, {
      product_name: nameB.toUpperCase(),
      product_name_status: 'active',
    }).then((res) => {
      expect(res.status).to.eq(409)
      expect(res.body.error).to.eq('Product name already exists')
  ...
  it('should allow updating to the same name the record already has (self-exclusion)', () => {
    // Updating a record with its own current name must NOT trigger a 409
    cy.UpdateProductName(idA, {
      product_name: nameA,
      product_name_status: 'active',
      note: 'Updated note — same name is valid (self-exclusion check)',
    }).then((res) => {
