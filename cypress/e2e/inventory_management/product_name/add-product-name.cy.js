...
describe('Product Name Add API and Database Comparison', () => {
  ...
  describe('Data Integrity - API vs Database Comparison', () => {
    ...
    beforeEach(() => {
      ...
      cy.AddProductName(request).then((response) => {
        cy.log('AddProductName response:', JSON.stringify(response.body))
        expect(response.status).to.eq(201)
        productNameId = response.body.productName.id
        userId = response.body.productName.user_id
        cy.log(`Created test Product Name ID: ${response.body.productName.id}`)
      })
    })
    ...
    describe('Total Active Product Names Impact', () => {
      it('should increment totalProductNames after creating a new product name', () => {
        cy.GetProductNameSummary().then((response) => {
          ...
          cy.log(`📊 Initial Product Name count: ${response.body.data.totalProductNames}`)
        })
        ...
        cy.GetProductNameSummary().then(function (response) {
          ...
          cy.log(`✅ Product Name count increased: ${this.initialCount} → ${newCount}`)
        })
      })
...
// Covers v1.14: uniqueness check (409 CONFLICT) on create, and UI inline error display
describe('Product Name Add — Uniqueness Check (409 Conflict)', () => {
  ...
  it('should return 409 when duplicate name differs only in case (case-insensitive)', () => {
    ...
    // Try the same name in upper-case
    cy.AddProductName({
  ...
  it('should allow creating the same name after it has been soft-deleted', () => {
    ...
    // Re-create should succeed because deleted records are excluded from uniqueness check
    cy.AddProductName({ product_name: name, product_name_status: 'active' }).then((res) => {
      expect(res.status).to.eq(201)
...
describe('Add Product Name Form - UI Tests', () => {
  describe('Desktop View', () => {
    ...
    describe('Dialog Behavior', () => {
      it('should open dialog when Add Product Name button is clicked', () => {
        cy.get('#addNewProductNameForm_productNamePage').should('not.exist')
        cy.get('#addNewProductNameBtn_productNamePage').should('be.visible').click()
        cy.get('#addNewProductNameForm_productNamePage').should('be.visible')
      })
  ...
  describe('Mobile View', () => {
    ...
    // Generated by Cypress Author — 2026-05-17
    // UI test: inline 409 conflict error shown as form field error (not toast)
    describe('Duplicate Name — 409 Inline Error (UI)', () => {
      it('should show inline field error when server returns 409 duplicate name', () => {
        ...
        // Seed a name via API so it already exists when we try the UI
        cy.setupApiAuthCookies()
        cy.AddProductName({
        ...
        // Intercept create endpoint to simulate the 409 response from server
        cy.intercept('POST', '/api/inventory/v1/product-name/create', {
          statusCode: 409,
        ...
        // Error must appear inline in the form field — NOT as a toast
        cy.get('#productNameField_errorMessage_productNamePage')
          .should('be.visible')
