...
describe("Product Name Detail API", () => {
    ...
    describe("Authentication & Authorization", () => {
        it("should return 401 for unauthenticated requests", () => {
            ...
            cy.GetProductNameDetailNoAuth(testProductNameId).then(
                (response) => {
    ...
    describe("Parameter Validation", () => {
        it("should return 400 for invalid product name ID (non-numeric)", () => {
            cy.GetProductNameDetail("invalid-id").then((response) => {
                ...
                expect(response.body.error).to.include(
                    "Product Name ID must be a valid number",
                );
            });
        });
        ...
        it("should return 400 for non-integer product name ID", () => {
            cy.GetProductNameDetail("123.45").then((response) => {
                ...
                expect(response.body.success).to.be.false;
                expect(response.body.error).to.eq(
                    "Product Name ID must be an integer",
                );
            });
        });
        it("should return 400 for ID = 0", () => {
            cy.GetProductNameDetail(0).then((response) => {
                ...
                expect(response.body.success).to.be.false;
                expect(response.body.error).to.eq(
                    "Product Name ID must be a positive integer",
                );
            });
        });
        ...
        it("should return correct Product Name data", () => {
            ...
            cy.GetProductNameDetail(testProductNameId).then((response) => {
                ...
                expect(apiProductName.product_name).to.eq(
                    responseProductName.product_name,
                );
                expect(apiProductName.product_name_status).to.eq(
                    responseProductName.product_name_status,
                );
    ...
    describe("Data Integrity", () => {
        it("should match database data exactly", () => {
            cy.GetProductNameDetail(testProductNameId)
                ...
                .then((apiData) => {
                    cy.getSingleProductNameFromDb(testProductNameId).then(
                        (dbResult) => {
                            const dbData = dbResult[0];

                            expect(dbData).to.exist;
                            expect(apiData.id).to.eq(dbData.id);
                            expect(apiData.product_name).to.eq(
                                dbData.product_name,
                            );
                            expect(apiData.product_name_status).to.eq(
                                dbData.product_name_status,
                            );
            ...
            productNameIds.forEach((request, index) => {
                request.then((response) => {
                    ...
                    if (index === 0) {
                        ...
                        expect(productName.product_name).to.eq(
                            firstproductName.product_name,
                        );
                        expect(productName.product_name_status).to.eq(
                            firstproductName.product_name_status,
                        );
