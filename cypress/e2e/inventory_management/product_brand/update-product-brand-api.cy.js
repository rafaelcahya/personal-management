...
describe('Product Brand Update API', () => {
  ...
  describe('Authentication & Authorization', () => {
    it('should update product brand successfully (200)', () => {
      ...
      cy.UpdateProductBrand(testProductBrandId, request).then((response) => {
  ...
  describe('Success Scenario', () => {
    it('should update product brand successfully (200)', () => {
      ...
      cy.UpdateProductBrand(testProductBrandId, request).then((response) => {
        ...
        exactFields.forEach((field) => {
          expect(response.body.productBrand[field]).to.eq(request[field])
        })
    ...
    it('updated_at should be more recent than created_at after update', () => {
      cy.UpdateProductBrand(testProductBrandId, {
        ...
        expect(new Date(updated_at).getTime()).to.be.gte(new Date(created_at).getTime())
      })
    })
  })
    ...
    describe('Complete Field Comparison', () => {
      it('should match all fields between API and DB', function () {
        ...
        cy.GetSingleProductBrand(productBrandId).then((response) => {
          ...
          cy.log('API product brand:', JSON.stringify(apiProductBrand))
        })

        cy.getSingleProductBrandFromDb(productBrandId.toString()).then((rows) => {
          dbProductBrand = rows[0]
          cy.log('DB product brand:', JSON.stringify(dbProductBrand))
  ...
  describe('Business Logic — brand_status deleted', () => {
    it('should auto-set deleted_at when brand_status is deleted', () => {
      cy.UpdateProductBrand(testProductBrandId, {
        ...
        expect(new Date(response.body.productBrand.deleted_at).toString()).to.not.eq('Invalid Date')
      })
    })
    ...
    it('should return 400 when deleted_at is invalid ISO date', () => {
      cy.UpdateProductBrand(testProductBrandId, {
        ...
        expect(response.status).to.eq(400)
        expect(response.body.success).to.be.false
        expect(response.body.error).to.include('deleted_at must be valid ISO date')
      })
    })
  ...
  describe('Request Body Validation', () => {
    ...
    it('should return 400 for invalid JSON string', () => {
      cy.UpdateProductBrand(testProductBrandId, 'NULL').then((response) => {
        expect(response.status).to.eq(400)
        expect(response.body.success).to.be.false
        expect(response.body.error).to.eq('Invalid JSON in request body')
      })
    ...
    it('should return 400 when brand_status is missing', () => {
      cy.UpdateProductBrand(testProductBrandId, {
        ...
        expect(response.body.success).to.be.false
        expect(response.body.error).to.be.an('array')
        expect(response.body.error).to.include('brand status is required')
      })
    })
  ...
  describe('Uniqueness Validation', () => {
    it('should return 409 when updating to a name that already exists', () => {
      const conflictingName = faker.food.fruit() + '-conflict-' + Date.now()

      cy.AddProductBrand({
        brand: conflictingName,
        brand_status: 'active',
        note: faker.word.words(5),
      }).then((res) => {
        expect(res.status).to.eq(201)
        cy.log(`✅ Conflicting brand created: "${conflictingName}"`)
        ...
        cy.UpdateProductBrand(testProductBrandId, {
          ...
          cy.log('❌ Conflict on update correctly rejected with 409')
        })
      })
    })

    it('should return 409 when updating to same name as another brand (case-insensitive)', () => {
      const conflictingName = faker.food.fruit() + '-ci-upd-' + Date.now()

      cy.AddProductBrand({
        brand: conflictingName.toLowerCase(),
        brand_status: 'active',
        note: faker.word.words(5),
      }).then((res) => {
        expect(res.status).to.eq(201)
        cy.log(`✅ Conflicting brand created: "${conflictingName.toLowerCase()}"`)

        cy.UpdateProductBrand(testProductBrandId, {
          brand: conflictingName.toUpperCase(),
          brand_status: 'active',
          note: faker.word.words(5),
        }).then((response) => {
          expect(response.status).to.eq(409)
          expect(response.body.success).to.be.false
          expect(response.body.error).to.eq('Brand name already exists')
          cy.log('❌ Case-insensitive conflict on update correctly rejected with 409')
        })
      })
    })

    it('should return 200 when updating brand to its own current name', () => {
      const ownName = faker.string.alphanumeric(10) + '-self-' + Date.now()
      ...
      cy.AddProductBrand({
        ...
        expect(res.status).to.eq(201)
        const freshId = res.body.productBrand.id

        cy.UpdateProductBrand(freshId, {
          brand: ownName,
          brand_status: 'active',
          note: faker.word.words(5),
        }).then((response) => {
          expect(response.status).to.eq(200)
          expect(response.body.success).to.be.true
          cy.log(`✅ Self-name update returned 200 — no false conflict for "${ownName}"`)
        })
      })
    })

    it('should return correct 409 response structure', () => {
      const conflictingName = faker.food.fruit() + '-struct-upd-' + Date.now()
      ...
      cy.AddProductBrand({
        ...
        cy.UpdateProductBrand(testProductBrandId, {
          ...
          cy.log('📊 409 response structure validated for update')
        })
      })
    })
  })
