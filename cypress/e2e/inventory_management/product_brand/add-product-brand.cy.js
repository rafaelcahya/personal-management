...
describe('Product Brand Add API and Database Comparison', () => {
    ...
    describe('Uniqueness Validation', () => {
      it('should return 409 when brand name already exists (exact match)', () => {
        ...
        cy.AddProductBrand({
          ...
          cy.log(`✅ First brand created: "${brandName}"`)

          // Attempt to create a duplicate with the same name
          cy.AddProductBrand({
            brand: brandName,
            brand_status: 'active',
            note: faker.word.words(5),
            ...
            cy.log('❌ Duplicate brand correctly rejected with 409')
          })
        })
      })
      ...
      it('should return 409 when brand name already exists (case-insensitive)', () => {
        ...
        cy.AddProductBrand({
          ...
          expect(res.status).to.eq(201)
          cy.log(`✅ First brand created: "${baseName.toLowerCase()}"`)
          ...
          cy.AddProductBrand({
            ...
            expect(response.body.success).to.be.false
            expect(response.body.error).to.eq('Brand name already exists')
            cy.log('❌ Case-insensitive duplicate correctly rejected with 409')
          })
        })
      })
      ...
      it('should return 201 when brand name is same as a deleted brand', () => {
        ...
        cy.AddProductBrand({
          ...
          cy.DeleteProductBrand(brandId).then((delRes) => {
            ...
            cy.log(`✅ Brand deleted, ID: ${brandId}`)
            ...
            cy.AddProductBrand({
              ...
              expect(response.status).to.eq(201)
              expect(response.body.success).to.be.true
              cy.log('✅ Brand recreated after deletion — 201 returned')
            })
          })
        })
      })
      ...
      it('should return correct 409 response structure', () => {
        ...
        cy.AddProductBrand({
          ...
          cy.AddProductBrand({
            ...
            cy.log('📊 409 response structure validated')
          })
        })
      })
    })
  ...
  describe('Data Integrity - API vs Database Comparison', () => {
    ...
    beforeEach(() => {
      ...
      cy.AddProductBrand(request).then((response) => {
        cy.log('AddProductBrand response:', JSON.stringify(response.body))
        expect(response.status).to.eq(201)
    ...
    describe('Total Active Product Brands Impact', () => {
      it('should increment totalProductBrands after creating a new product brand', () => {
        cy.GetProductBrandSummary().then((response) => {
          ...
          cy.log(`📊 Initial Product Brand count: ${response.body.data.totalProductBrands}`)
        })
        ...
        cy.GetProductBrandSummary().then(function (response) {
          ...
          cy.log(`✅ Product Brand count increased: ${this.initialCount} → ${newCount}`)
        })
      })
...
describe('Add Product Brand Form - UI Tests', () => {
  describe('Desktop View', () => {
    ...
    describe('Dialog Behavior', () => {
      ...
      it('should close dialog when Cancel button is clicked', () => {
        cy.get('#addNewProductBrandBtn_desktop_productBrandPage').click()
        cy.wait(1000)
        cy.get('#addNewProductBrandForm_desktop_productBrandPage').should('be.visible')
  ...
  describe('Tablet View', () => {
    ...
    describe('Dialog Behavior', () => {
      it('should open dialog when Add Product Brand button is clicked', () => {
        cy.get('#addNewProductBrandForm_desktop_productBrandPage').should('not.exist')
        cy.get('#addNewProductBrandBtn_desktop_productBrandPage').should('be.visible').click()
        cy.get('#addNewProductBrandForm_desktop_productBrandPage').should('be.visible')
      })
