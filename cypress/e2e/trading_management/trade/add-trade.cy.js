  ...
  describe('Add Trade API', () => {
    ...
    describe('Authentication & Authorization', () => {
      it('should return 307 or 401 when user is not authenticated', () => {
        ...
        const request = {
          buy_date: faker.date.recent({ days: 10 }).toISOString().split('T')[0],
          sell_date: faker.date.recent({ days: 5 }).toISOString().split('T')[0],
    ...
    describe('Required Fields Validation', () => {
      it('should return 400 when ticker is missing', () => {
        ...
        cy.AddTrade(request).then((response) => {
          ...
          expect(response.body.success).to.be.false
          expect(response.body.error).to.be.an('array')
          expect(response.body.error).to.include('ticker is required')
        })
      })
      ...
      it('should return 400 when margin is missing', () => {
        ...
        cy.AddTrade(request).then((response) => {
          ...
          expect(response.body.success).to.be.false
          expect(response.body.error).to.be.an('array')
          expect(response.body.error).to.include('margin is required')
        })
      })
      ...
      it('should return 400 when proceeds is missing', () => {
        ...
        cy.AddTrade(request).then((response) => {
          ...
          expect(response.body.success).to.be.false
          expect(response.body.error).to.be.an('array')
          expect(response.body.error).to.include('proceeds is required')
        })
      })
      ...
      it('should return 400 with multiple validation errors', () => {
        ...
        cy.AddTrade(request).then((response) => {
          ...
          cy.log(`Validation errors: ${response.body.error.join(', ')}`)
        })
      })
    })
    ...
    describe('Trade Object Structure Scenarios', () => {
      it('should create trade with all required fields', () => {
        ...
        cy.AddTrade(request).then((response) => {
          ...
          expect(trade).to.have.property('buy_date')
          expect(trade).to.have.property('sell_date')
    ...
    describe('Success Scenarios', () => {
      ...
      it('should generate timestamps (created_at, updated_at)', () => {
        ...
        cy.AddTrade(request).then((response) => {
          ...
          expect(new Date(trade.created_at).toString()).to.not.eq('Invalid Date')
    ...
    describe('Complete Field Comparison', () => {
      it('should match all fields between API and DB', () => {
        ...
        cy.then(() => {
          ...
          expect(apiTrade.trade_date, 'Trade Date').to.eq(dbTrade.trade_date)
          expect(apiTrade.user_id, 'User ID').to.eq(dbTrade.user_id)
          expect(apiTrade.margin, 'Margin').to.eq(dbTrade.margin)
          expect(apiTrade.proceeds, 'Proceeds').to.eq(dbTrade.proceeds)
          expect(apiTrade.realized_gain, 'Realized Gain').to.eq(dbTrade.realized_gain)
          expect(apiTrade.return_percent, 'Return Percent').to.eq(dbTrade.return_percent)
          expect(apiTrade.stock_type_option, 'Stock Type').to.eq(dbTrade.stock_type_option)
          expect(apiTrade.entry_session_option, 'Entry Session').to.eq(dbTrade.entry_session_option)
          expect(apiTrade.entry_occasion_option, 'Entry Occasion').to.eq(
            dbTrade.entry_occasion_option
          )
          expect(apiTrade.buy_reason_option, 'Buy Reason').to.eq(dbTrade.buy_reason_option)
          expect(apiTrade.sell_reason_option, 'Sell Reason').to.eq(dbTrade.sell_reason_option)
          expect(apiTrade.notes, 'Notes').to.eq(dbTrade.notes)
          expect(apiTrade.created_at, 'Created At').to.eq(dbTrade.created_at)
          expect(apiTrade.updated_at, 'Updated At').to.eq(dbTrade.updated_at)
          expect(apiTrade.deleted_at, 'Deleted At').to.eq(dbTrade.deleted_at)
      ...
      it('should match realized_gain including negative values', () => {
        ...
        cy.then(() => {
          if (apiTrade.realized_gain && dbTrade.realized_gain) {
            ...
            expect(apiTrade.realized_gain).to.eq(dbTrade.realized_gain)
    ...
    describe('Total Trades Count Impact', () => {
      it('should increment totalTrades after creating a new trade', () => {
        ...
        cy.GetTradeSummary().then((response) => {
          ...
          cy.log(`✅ Total trades increased: ${initialCount} → ${newCount}`)
        })
      })
    ...
    describe('Loss Count Impact', () => {
      it('should increment totalLosses when creating loss trade', () => {
        ...
        cy.GetTradeSummary().then((response) => {
          ...
          cy.log(`✅ Losses increased: ${initialLosses} → ${newLosses}`)
        })
      })
    ...
    describe('Win Rate Calculation Impact', () => {
      it('should recalculate win rate after adding win trade', () => {
        ...
        cy.GetTradeSummary().then((response) => {
          ...
          const initialWinRate =
            initialSummary.totalTrades > 0
              ? (initialSummary.totalWins / initialSummary.totalTrades) * 100
              : 0
          cy.log(`📊 Initial win rate: ${initialWinRate.toFixed(1)}%`)
        })
        ...
        cy.GetTradeSummary().then((response) => {
          ...
          const expectedWinRate = (newSummary.totalWins / newSummary.totalTrades) * 100

          const actualWinRate = (newSummary.totalWins / newSummary.totalTrades) * 100
    ...
    describe('Stock Type Summary Impact', () => {
      it('should add new stock type to summary when created', () => {
        ...
        cy.GetTradeSummary().then((response) => {
          ...
          cy.log(`📊 Initial ${request.stock_type_option} count: ${initialCount}`)
        })
        ...
        cy.GetTradeSummary().then((response) => {
          ...
          expect(newCount).to.eq(initialCount + 1)
          cy.log(`✅ ${request.stock_type_option} count increased: ${initialCount} → ${newCount}`)
        })
      })
    ...
    describe('Entry Session Summary Impact', () => {
      it('should update entry session summary when trade created', () => {
        ...
        cy.GetTradeSummary().then((response) => {
          ...
          cy.log(`📊 Initial ${request.entry_session_option} count: ${initialCount}`)
        })
    ...
    describe('Entry Occasion Summary Impact', () => {
      it('should update entry occasion summary when trade created', () => {
        ...
        cy.GetTradeSummary().then((response) => {
          ...
          cy.log(`📊 Initial ${request.entry_occasion_option} count: ${initialCount}`)
        })
    ...
    describe('Complete Summary Update Verification', () => {
      it('should update all metrics for PROFIT trade', () => {
        ...
        cy.GetTradeSummary().then((response) => {
          ...
          expect(newSummary.totalTrades).to.eq(initialSummary.totalTrades + 1)
          expect(newSummary.totalWins).to.eq(initialSummary.totalWins + 1)
          expect(newSummary.totalLosses).to.eq(initialSummary.totalLosses)
          ...
          expect(newSummary.entryOccasionSummary[occasion]).to.eq(
            (initialSummary.entryOccasionSummary[occasion] || 0) + 1
          )
      ...
      it('should update all metrics for LOSS trade', () => {
        ...
        cy.GetTradeSummary().then((response) => {
          ...
          expect(newSummary.totalTrades).to.eq(initialSummary.totalTrades + 1)
          expect(newSummary.totalWins).to.eq(initialSummary.totalWins)
          expect(newSummary.totalLosses).to.eq(initialSummary.totalLosses + 1)
  ...
  describe('Desktop View', () => {
    ...
    describe('Dialog Behavior', () => {
      it('should open dialog when Add Trade button is clicked', () => {
        ...
        cy.get('#addNewTradeBtn_tradePage').should('be.visible').click()
    ...
    describe('Required Fields Validation', () => {
      ...
      it('should show error when margin is empty', () => {
        cy.get('#tickerField_tradePage').type(faker.word.noun(4).toUpperCase())
        cy.get('#submitNewTradeBtn_tradePage').click()
    ...
    describe('Field Input Behavior', () => {
      ...
      it('should accept numeric input for margin', () => {
        ...
        cy.get('#marginField_tradePage').should('have.value', formatToRupiah(margin))
      })
      ...
      it('should accept numeric input for proceeds', () => {
        ...
        cy.get('#proceedsField_tradePage').should('have.value', formatToRupiah(margin))
      })
    ...
    describe('Trade Date Picker', () => {
      ...
      it('should select a date from calendar', () => {
        cy.get('#tradeDateField_tradePage').click()
        cy.get('#tradeDatePicker_tradePage').contains('button', '15').click()
        cy.get('#tradeDateField_tradePage').should('not.contain', 'Pick a date')
      })

      it('should display selected date in correct format', () => {
        cy.get('#tradeDateField_tradePage').click()
        cy.get('#tradeDatePicker_tradePage').contains('button', '10').click()
      ...
      it('should format large numbers correctly', () => {
        ...
        cy.get('#marginField_tradePage').should('have.value', formatToRupiah(largeValue))
      })
      ...
      it('should handle different value ranges', () => {
        ...
        testCases.forEach(({ input, expected }) => {
          ...
          cy.get('#marginField_tradePage').should('have.value', expected)
        })
      })
    })
    ...
    describe('Auto-Calculation', () => {
      ...
      it('should update calculations when values change', () => {
        ...
        cy.wait(300)
        cy.get('#realizedGainValue_realizedGainField_tradePage').should('have.value', 'Rp. 500.000')
      })
      ...
      it('should clear calculations when fields are empty', () => {
        ...
        cy.wait(300)
        cy.get('#realizedGainValue_realizedGainField_tradePage').should('have.value', 'Rp. 0')
        cy.get('#returnPercentValue_returnPercentField_tradePage').should('have.value', '')
      })
    })
  })
