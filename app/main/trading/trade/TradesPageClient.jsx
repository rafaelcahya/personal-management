...
export default function TradesPageClient({ initialTrades }) {
  ...
  const fetchTrades = useCallback(async () => {
    try {
      setIsLoading(true)
      const [trades] = await Promise.all([fetchTradeList(), fetchSummary()])
      setListTrade(trades || [])
    } catch (err) {
        ...
        <div className="flex flex-col gap-5 sm:gap-0 h-full overflow-hidden">
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-sm text-slate-600 font-medium">Loading trades...</p>
            </div>
          ) : listTrade.length === 0 ? (
            ...
            <TradesTable trades={listTrade} onTradesChange={setListTrade} onRefresh={fetchTrades} />
          )}
        </div>
      </div>
    </div>
