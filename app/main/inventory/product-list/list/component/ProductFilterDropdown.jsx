...
export default function ProductFilterDropdown({
    ...
    const getFilterCount = (filterValue) => {
        ...
        const counts = {
            null: summary.totalProducts,
            active: summary.activeProducts,
            inactive: summary.inactiveProducts,
            favorite: summary.favoriteProducts,
            "low-stock": summary.lowStockProducts,
            "out-stock": summary.outOfStockProducts,
            "never-used": summary.neverUsedProducts,
        };
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-64">
                ...
                {/* Status Group */}
                {FILTER_OPTIONS.filter((opt) => opt.group === "status").map(
                    (option) => (
                        <DropdownMenuItem
                            key={option.value}
                ...
                {/* Inventory Group */}
                {FILTER_OPTIONS.filter((opt) => opt.group === "inventory").map(
                    (option) => (
                        <DropdownMenuItem
                            key={option.value}
                            onClick={() => onFilterChange(option.value)}
                ...
                {/* Usage Group */}
                {FILTER_OPTIONS.filter((opt) => opt.group === "usage").map(
                    (option) => (
                        <DropdownMenuItem
                            key={option.value}
                            onClick={() => onFilterChange(option.value)}
