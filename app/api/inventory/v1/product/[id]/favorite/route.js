...
export async function PATCH(req, { params }) {
  try {
    ...
    if (isFavorite === undefined || isFavorite === null) {
      return NextResponse.json({ success: false, error: 'isFavorite is required' }, { status: 400 })
    }
    ...
    const updatedProduct = await favoriteProduct(user.id, idNum, isFavorite)

    return NextResponse.json({ success: true, data: updatedProduct }, { status: 200 })
  } catch (err) {
    console.error('PATCH /api/inventory/v1/product/[id]/favorite error:', err)
